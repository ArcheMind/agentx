...
AgentX shows a loading status while it performs bounded recent-session discovery. Results are recent-first, with at most ten sessions per provider in each group. Each row displays provider, local time, workspace, and title in that order. The workspace column is empty for Current workspace and shows a compact, suffix-preserving path for Global sessions, or `unknown` when no workspace is available.
