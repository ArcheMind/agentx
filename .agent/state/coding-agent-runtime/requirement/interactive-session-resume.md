...
Bare `ax` and `ax session list` must exclude provider-identified sub-Agent sessions by default so child execution traces do not crowd the primary recovery surface. An explicit `--include-subagents` option includes them and summaries expose normalized sub-Agent status. Filtering must not make a known child session unreachable: direct `Info` and resume by explicit ID remain supported. Classification must use reliable native session metadata; providers without a reliable session-level marker must not be guessed.
