...
Titles must prefer actual user tasks over injected Agent context, and all truncation must preserve UTF-8 characters. Times use local, relative-friendly labels. Session rows use four columns in this order: provider, time, workspace, title. The time column is limited to 15 characters; the workspace column is limited to 20 characters and left-truncates to preserve the path suffix; the final title column is limited to 45 characters. Current-workspace rows leave the workspace column empty, while Global rows explicitly show `unknown` when unavailable. The selector must remain within a 96-column terminal without corrupting redraw line counts.
