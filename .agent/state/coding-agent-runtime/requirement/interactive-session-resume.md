...
Titles must prefer actual user tasks over injected Agent context, and all truncation must preserve UTF-8 characters. Times use local, relative-friendly labels. Global rows inline a compact workspace label alongside the title on the same line, explicitly showing `unknown` when unavailable, and the selector must remain within a 96-column terminal without corrupting redraw line counts.
